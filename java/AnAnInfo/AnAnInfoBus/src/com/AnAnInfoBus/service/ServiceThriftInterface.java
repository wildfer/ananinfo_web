...
import java.util.*;
import com.AnAnInfoBus.business.Interface.Jsoninterface;
import com.AnAnInfoBus.thrift.*;
...
//    }
    ...
    public ParamsOut EcShopJson(ParamsIn strJsonIn) throws TException {
        ParamsOut r=new ParamsOut(0,"success","true");
        try {
        	Jsoninterface jsoninterface = new Jsoninterface();      	
        	r.setJsonOut(jsoninterface.Process(strJsonIn.getStrCmdID(), strJsonIn.getJsonIn()));
        }catch (CustomException e) {
            r=new ParamsOut(e.getCode(),e.getMessage(),"false");
            if(e.getCode()!=CustomException.USER_NOT_SIGN_IN
                    && e.getCode()!=CustomException.INVALID_USER_ID){
                e.printStackTrace();
                LOGGER.error(e.getMessage()+e.getCode() );
            }
            ...
            e.printStackTrace();
            r=new ParamsOut(CustomException.SYS_ERROR,"系统繁忙,请稍后再试","false");
            LOGGER.error(e.getMessage());
        }
        return r;
    }
